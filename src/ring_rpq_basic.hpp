    ...
    /* void next_step_const_to_var_verbose(RpqAutomata &A, std::vector<word_t> &B_array,
                                         ...
                                         bool const_to_var = true)
     {
         ...
         for (uint64_t i = 0; i < pred_vec.size(); i++)
         {
             ...
             std::vector<std::tuple<uint64_t, word_t, std::pair<uint64_t, uint64_t>>> subj_vec;
             L_S.all_active_s_values_in_range_test<word_t>(c + interval.first, c + interval.second,
                                                           D_array, (word_t)A.next(current_D, pred_vec[i].first, BWD), subj_vec);
 ...
 */
    ...
    bool _rpq_const_s_to_var_o(RpqAutomata &A,
                               ...
                               std::vector<std::pair<uint64_t, uint64_t>> &solutions,
                               bool const_to_var,
                               high_resolution_clock::time_point start, bool is_empty_path_solution)
    {
    ...
    bool _2rpq_const_s_to_var_o(RpqAutomata &A,
                                std::unordered_map<std::string, uint64_t> &predicates_map,
                                std::vector<word_t> &B_array,
                                uint64_t initial_object,
                                std::vector<std::pair<uint64_t, uint64_t>> &solutions,
...
public:
    /*   void or_query_var_to_var(const std::string &rpq, uint64_t n_or,
       ...
       {
           uint64_t i, i1, pred,  k, start, a, max_pos;
           bool negated_pred;
...
*/
  ...
  */
                       ...
                       uint64_t n_predicates, bool is_negated_pred, uint64_t n_operators, bool is_a_path, bool is_const_to_var)
    {
        ...
        if (is_const_to_var)
            _rpq_const_s_to_var_o(A, predicates_map, B_array, initial_object, output_subjects, true, start, false);
        else
            _rpq_const_s_to_var_o(A, predicates_map, B_array, initial_object, output_subjects, false, start, false);
                         ...
                         uint64_t n_predicates, bool is_negated_pred, uint64_t n_operators, bool is_a_path)
    {
        ...
        // cout << " objetos encontrados para caso ?x?y" << endl;
        ...
        {
            // cout << "OBJETO - " << objects_var_to_var[i] << endl;
            _rpq_const_s_to_var_o(A2, predicates_map, B_array, objects_var_to_var[i], output_subjects, true, start, empty_path_is_solution);
        }
